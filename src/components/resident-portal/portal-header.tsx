...
import { ResidentGlobalSearch } from './resident-global-search';
...
export function PortalHeader() {
  ...
  // Determine greeting based on mode? Or generic?
  // Spec says: "Search ... Suggest Actions relevant to context"
    ...
    <>
      ...
      >
        <div className="h-full px-6 flex items-center justify-between gap-4">
          ...
          {/* Context Switcher (Visible to mixed-role users) */}
          ...
          <div className="hidden md:flex flex-1 max-w-md mx-auto">
            <div
              className="search-input-wrapper w-full cursor-pointer hover:bg-muted/50 transition-colors"
              onClick={() => setSearchOpen(true)}
            >
              <Search className="search-icon" />
              <div
                className="search-input flex items-center text-muted-foreground"
                style={{ fontSize: '0.875rem' }}
              >
                Quick Search... <span className="ml-auto text-xs opacity-50">⌘K</span>
              </div>
            </div>
          </div>
      ...
      <ResidentGlobalSearch open={searchOpen} onOpenChange={setSearchOpen} />
    </>
  );
}
