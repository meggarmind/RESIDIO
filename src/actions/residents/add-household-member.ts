 ...
 * - co_resident: Occupants (adults living in unit, not on title/lease)
 * - household_member: Family members, dependents living in the house
 * - domestic_staff: Domestic workers (needs sponsor)
 * - caretaker: Assigned to maintain the unit (added by property owners/developers)
 * - contractor: External service providers (added by property owners/developers)
 */
const ALLOWED_HOUSEHOLD_ROLES: ResidentRole[] = ['co_resident', 'household_member', 'domestic_staff', 'caretaker', 'contractor'];
...
const HouseholdMemberSchema = z.object({
  ...
  resident_role: z.enum(['co_resident', 'household_member', 'domestic_staff', 'caretaker', 'contractor']),
...
export async function addHouseholdMember(
  ...
  if (!ALLOWED_HOUSEHOLD_ROLES.includes(resident_role)) {
    return { data: null, error: 'Invalid role. Only occupants, household members, domestic staff, caretakers, and contractors can be added.' };
  }
  ...
  // Verify caller's role and permissions for this house
  const { data: callerAssignment, error: roleCheckError } = await supabase
    .from('resident_houses')
    .select('id, is_primary, resident_role')
    .eq('resident_id', profile.resident_id)
    .eq('house_id', house_id)
    .eq('is_active', true)
    .single();

  if (roleCheckError || !callerAssignment) {
    return { data: null, error: 'You are not assigned to this property' };
  }

  // Define role-based permissions according to docs/architecture/resident-types-roles.md
  const OCCUPIER_ROLES = ['resident_landlord', 'tenant']; // Owner-Occupier and Renter
  const OWNER_ROLES = ['non_resident_landlord', 'developer']; // Property Owner and Developer

  const OCCUPIER_CAN_ADD = ['co_resident', 'household_member', 'domestic_staff'];
  const OWNER_CAN_ADD = ['caretaker', 'contractor'];

  // Check caller's role and permissions for the specific member type
  const callerRole = callerAssignment.resident_role;
  const canAdd =
    (OCCUPIER_ROLES.includes(callerRole) && OCCUPIER_CAN_ADD.includes(resident_role)) ||
    (OWNER_ROLES.includes(callerRole) && OWNER_CAN_ADD.includes(resident_role));

  if (!canAdd) {
    if (OCCUPIER_ROLES.includes(callerRole)) {
      return { data: null, error: 'As an occupying resident, you can only add occupants, family members, and domestic staff' };
    } else if (OWNER_ROLES.includes(callerRole)) {
      return { data: null, error: 'As a property owner, you can only add caretakers and contractors' };
    } else {
      return { data: null, error: 'Your resident role does not have permission to add household members' };
    }
  }
  ...
  const { data: newResident, error: createError } = await supabase
    ...
    .insert({
      first_name,
      last_name,
      phone_primary: phone_primary || '', // Empty string if not provided (database requires non-null)
      email: email || null,
      resident_type: 'secondary', // Secondary residents (household members, staff, etc.)
