...
Audit logs can be viewed at `/settings/audit-logs` by users with the `settings.view_audit_logs` permission. The `super_admin` and `vice_chairman` roles hold it; `chairman` does not.
