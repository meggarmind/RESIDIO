...
export function BankPasswordsSection() {
  ...
  // Filter to only show First Bank accounts
  const firstBankAccounts = accounts?.filter((a) =>
    a.bank_name.toLowerCase().includes('first')
  );
    ...
    <>
      <Card>
        ...
        <CardContent>
          ...
          ) : !firstBankAccounts || firstBankAccounts.length === 0 ? (
            <div className="text-center py-4 text-muted-foreground">
              No First Bank accounts found. Add bank accounts in Estate Settings.
            </div>
          ) : (
            <div className="space-y-4">
              {firstBankAccounts.map((account) => (
                <div
                  key={account.id}
