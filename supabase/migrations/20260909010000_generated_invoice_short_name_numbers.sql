...
BEGIN
    ...
    BEGIN
        ...
        SELECT NULLIF(btrim(short_name), '') INTO v_house_short_name
        FROM public.houses
        WHERE id = v_candidate.house_id;

        v_invoice_number := CASE
            WHEN v_house_short_name IS NOT NULL THEN format(
                'INV-%s-%s-%s',
                v_house_short_name,
                to_char(v_candidate.period_start, 'YYYY'),
                to_char(v_candidate.period_start, 'MM')
            )
            ELSE format(
                'INV-%s-%s',
