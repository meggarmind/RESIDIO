...
-- Note: Removed NOW() from partial index predicates (not allowed - non-immutable)
...
-- Partial index on expires_at IS NULL (non-expiring notifications)
CREATE INDEX IF NOT EXISTS idx_notifications_recipient_category
ON in_app_notifications(recipient_id, category, is_read)
WHERE expires_at IS NULL;

-- Index for expired notifications (separate index for queries)
CREATE INDEX IF NOT EXISTS idx_notifications_recipient_category_expiring
ON in_app_notifications(recipient_id, category, is_read, expires_at)
...
'Optimizes notification center filtering by category and read status (non-expiring)';
