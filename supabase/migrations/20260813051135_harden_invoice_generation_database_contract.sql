...
BEGIN
    IF OLD.is_locked THEN
        RAISE EXCEPTION 'Locked billing profile version % is immutable', OLD.id USING ERRCODE = '55000';
    END IF;
...
BEGIN
    IF EXISTS (
        SELECT 1 FROM public.billing_profile_versions
        WHERE id = OLD.billing_profile_version_id AND is_locked
    ) THEN
        RAISE EXCEPTION 'Items for locked billing profile version % are immutable', OLD.billing_profile_version_id USING ERRCODE = '55000';
    END IF;
    RETURN CASE WHEN TG_OP = 'DELETE' THEN OLD ELSE NEW END;
END;
$$;

DROP TRIGGER IF EXISTS prevent_locked_billing_profile_version_mutation ON public.billing_profile_versions;
CREATE TRIGGER prevent_locked_billing_profile_version_mutation
    BEFORE UPDATE OR DELETE ON public.billing_profile_versions
    FOR EACH ROW EXECUTE FUNCTION public.prevent_locked_billing_profile_version_mutation();

DROP TRIGGER IF EXISTS prevent_locked_billing_profile_version_item_mutation ON public.billing_profile_version_items;
CREATE TRIGGER prevent_locked_billing_profile_version_item_mutation
    BEFORE UPDATE OR DELETE ON public.billing_profile_version_items
    FOR EACH ROW EXECUTE FUNCTION public.prevent_locked_billing_profile_version_item_mutation();

ALTER TABLE public.invoice_generation_candidates
    DROP CONSTRAINT IF EXISTS invoice_generation_candidates_run_id_resident_id_house_id_b_key;
ALTER TABLE public.invoice_generation_candidates
    ADD CONSTRAINT invoice_generation_candidates_business_key
    UNIQUE NULLS NOT DISTINCT (run_id, resident_id, house_id, billing_profile_version_id, period_start, period_end);

DROP INDEX IF EXISTS public.invoices_generated_business_key;
...
BEGIN
    ...
    BEGIN
        ...
        IF v_item_total IS NULL OR v_item_total <> v_candidate.amount_due THEN
            RAISE EXCEPTION 'Invoice generation candidate % item total % does not equal amount due %', p_candidate_id, v_item_total, v_candidate.amount_due;
        END IF;
