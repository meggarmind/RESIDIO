...
- **Verification in `.worktrees/issue-147`:** `npm run lint` passes with 0 errors / 330 warnings; `npm test -- --run` passes 361 tests in 62 files; `npm run build` passes when the existing ignored root `.env.local` is loaded for Supabase prerendering. The worktree intentionally has no `.env.local`; an unconfigured build correctly fails while prerendering `/settings/document-categories` because Supabase credentials are absent.
