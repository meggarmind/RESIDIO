...
- **Verification in `.worktrees/issue-147`:** `npm run lint` passes with 0 errors / 330 warnings; `npm test -- --run` passes 360 tests in 62 files; `npm run build` passes when the existing ignored root `.env.local` is loaded for Supabase prerendering. The worktree intentionally has no `.env.local`; an unconfigured build correctly fails while prerendering `/settings/document-categories` because Supabase credentials are absent.
...
- The issue workflow helper now normalizes worktree paths before comparing them, fixing its false rejection of Windows worktrees reported with forward slashes by Git. The focused workflow test covers slash normalization.
