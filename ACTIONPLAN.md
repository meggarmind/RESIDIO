...
- Final verification in `.worktrees/issue-147`: `npm run lint` passes with 0 errors / 330 warnings, `npm test -- --run` passes 361/361, and `npm run build` passes when the existing ignored root `.env.local` is loaded for Supabase prerendering. The worktree intentionally has no `.env.local`; without it, prerendering correctly fails due to missing Supabase credentials. #147 lifecycle review passed; merge and issue completion remain next.
