...
- Final verification in `.worktrees/issue-147`: `npm run lint` passes with 0 errors / 330 warnings, `npm test -- --run` passes 360/360, and `npm run build` passes when the existing ignored root `.env.local` is loaded for Supabase prerendering. The worktree intentionally has no `.env.local`; without it, prerendering correctly fails due to missing Supabase credentials. Review, merge, and issue lifecycle completion remain next.
